    ...
    return (
        <header
            className={cn(
                ...
                scrolled
                    ? "bg-cream/95 backdrop-blur-md shadow-[0_2px_20px_rgba(74,124,89,0.08)]"
                    : "bg-transparent"
            )}
        >
            <div className="container mx-auto px-4 md:px-6 flex items-center justify-between h-18">
                ...
                <button
                    ...
                    className="lg:hidden p-2 rounded-full text-earth hover:bg-warm transition-colors"
                >
            ...
            {isOpen && (
                <div className="absolute top-full left-0 right-0 bg-cream border-t border-sand shadow-lg p-5 lg:hidden flex flex-col gap-3">
                    {navItems.map((item) => (
                        <Link
                            key={item.href}
                            href={item.href}
                            onClick={() => setIsOpen(false)}
                            className="text-base font-medium text-earth/80 py-2.5 border-b border-sand/50 last:border-0 hover:text-primary transition-colors"
                        >
                            {item.label}
                        </Link>
                    ))}
